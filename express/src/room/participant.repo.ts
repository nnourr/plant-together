...
export class RoomParticipantRepo{
  ...
  async addUserAccess(roomId: string, userId: string) {
    if (!userId || !roomId) throw new Error(`Failed to add user ${userId} to room ${roomId}`);
    await this.sql`INSERT INTO room_participant (room_id, user_id) VALUES (${roomId}, ${userId}) ON CONFLICT DO NOTHING`;
  }
}
