...
export class RoomService {
    ...
    constructor(
        ...
        signed: (options: SignatureOptions) => Signature,
    ) {
    ...
    async getUML(roomId: string) {
        ...
        for (let document of room_documents) {
            content.push({
                ...
                uml: await this.documentRepo.getDocumentUML(
                    roomId,
                    document.id,
                ),
            })
    ...
    async validatePublicRoomName(roomName: string) {
        try {
            const roomid = await this.roomRepo.retrieveRoomIdByAccess(
                roomName,
                false,
            )
            if (!roomid) {
    ...
    async processRoomSignature(
        ...
        try {
            ...
            if (verifiedRoomId !== roomId)
                throw new Error(
                    "Provided signature doesn't match requested room",
                )
