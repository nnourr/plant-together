...
describe("Socket.IO Documents Rename Functionality", () => {
    ...
    beforeEach(async () => {
        clientSocket = ClientSocket(`http://localhost:${PORT2}/documents`, { extraHeaders: { 'room-id': DEFAULT_ROOM_ID } });
        clientSocket.on("connect", () => expect(clientSocket.connected).toBe(true));
    ...
    test("should notify other clients when a document is renamed", (done) => {
        ...
        const clientSocket2 = ClientSocket(`http://localhost:${PORT2}/documents`, { extraHeaders: { 'room-id': DEFAULT_ROOM_ID } });
