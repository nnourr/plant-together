...
app.get('/room/private/:owner_id/:room_name', async (req, res) => {
    ...
    if (!roomName)
        return res.status(400).json({ error: 'No Room name Specified' })
    if (!ownerId)
        return res.status(400).json({ error: 'No owner ID Specified' })
    ...
    try {
        ...
        try {
            ...
            logger.error(
                `Failed to process provided signature. Continuing with existing user permissions`,
            )
        }
...
app.post('/room/:room_name', async (req, res) => {
    ...
    try {
        if ((await authService.isGuestUser(token!)) && is_private) {
            return res
                .status(403)
                .json({ error: 'Guest Users Cannot Create Private Rooms' })
        }

        if (
            !is_private &&
            !(await roomService.validatePublicRoomName(room_name))
        ) {
            return res
                .status(400)
                .json({ error: 'Public room name already exists' })
        }
...
app.put('/room/:room_id/access', async (req, res) => {
    ...
    if (is_private === undefined)
        return res.status(400).json({ error: 'No is private provided' })

    try {
        if (!(await roomService.validateRoomCreator(token!, room_id))) {
            return res
                .status(403)
                .json({ error: "Guest user can't change access" })
        }
