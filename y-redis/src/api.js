...
const extractMessagesFromStreamReply = (streamReply, prefix) => {
    ...
    streamReply?.forEach(docStreamReply => {
        ...
        docStreamReply.messages.forEach(m => {
            if (m.message.m != null) {
                docMessages.messages.push(
                    /** @type {Uint8Array} */ (m.message.m),
                )
            }
        })
    })
...
export class Api {
    ...
    addMessage(room, docid, m) {
        ...
        if (
            m[0] === protocol.messageSync &&
            m[1] === protocol.messageSyncStep2
        ) {
            if (m.byteLength < 4) {
                ...
                if (streamlen === 0) {
                    ...
                    await promise.all([
                        ...
                        this.redis
                            ...
                            .xAdd(this.redisWorkerStreamName, '*', {
                                compact: task.stream,
                            })
                            .xReadGroup(
                                this.redisWorkerGroupName,
