    ...
    app.ws(
        ...
        /** @type {uws.WebSocketBehavior<User>} */ ({
            ...
            upgrade: async (res, req, context) => {
                ...
                const headerWsExtensions = req.getHeader(
                ...
                try {
                    const { hasWriteAccess, room, userid } =
                        await checkAuth(req)
                    if (aborted) return
                    res.cork(() => {
                        res.upgrade(
            ...
            open: async ws => {
                ...
                ws.cork(() => {
                    ws.send(
                        protocol.encodeSyncStep1(
                            Y.encodeStateVector(indexDoc.ydoc),
                        ),
                        true,
                        false,
                    )
                    ws.send(
                        protocol.encodeSyncStep2(
                            Y.encodeStateAsUpdate(indexDoc.ydoc),
                        ),
                        true,
                        true,
                    )
