...
describe('DownloadModal Component', () => {
    beforeEach(() => {
        ...
        ;(getRoomUML as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(
            mockUMLContents,
        )
        ;(
            plantuml.renderSvg as unknown as ReturnType<typeof vi.fn>
        ).mockResolvedValue('<svg>test</svg>')
        ;(
            plantuml.renderPng as unknown as ReturnType<typeof vi.fn>
        ).mockResolvedValue({ blob: new Blob(), error: undefined })
    })
    ...
    describe('Initial Render', () => {
        ...
        test('renders all filter buttons', () => {
            ...
            expect(
                screen.getByRole('button', { name: /code only/i }),
            ).toBeInTheDocument()
            expect(
                screen.getByRole('button', { name: /svg only/i }),
            ).toBeInTheDocument()
            expect(
                screen.getByRole('button', { name: /png only/i }),
            ).toBeInTheDocument()
        })
        ...
        test('renders all documents with correct initial state', () => {
            ...
            sampleDocuments.forEach(doc => {
                ...
                const checkboxForDoc = checkbox.find(
                    cb => cb.getAttribute('name') === doc.name,
                )
                ...
                formatTypes.forEach(format => {
                    const formatElement = screen
                        .getAllByText(format)
                        .find(el =>
                            el.parentElement?.className.includes(
                                'flex gap-2 items-center mr-3',
                            ),
                        )
                    expect(formatElement).toBeInTheDocument()
                    expect(formatElement?.closest('span')).not.toHaveClass(
                        'line-through',
                    )
                })
            })
    ...
    describe.skip('Filter Functionality', () => {
        test('Code Only filter selects only code format', () => {
            ...
            sampleDocuments.forEach(doc => {
                ...
                expect(codeElement?.closest('span')).not.toHaveClass(
                    'line-through',
                )
    ...
    describe('Document Selection', () => {
        test('toggles document selection when checkbox is clicked', async () => {
            ...
            const checkbox = checkboxes.filter(
                cb => cb.getAttribute('name') === 'Document 1',
            )[0]
    ...
    describe('Download Functionality', () => {
        ...
        test('handles download errors gracefully', async () => {
            ...
            ;(
                getRoomUML as unknown as ReturnType<typeof vi.fn>
