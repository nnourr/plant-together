...
describe('UmlEditor', () => {
  ...
  test('updates decorations when error prop changes', async () => {
    ...
    await waitFor(() => {
      ...
      expect(rangeMock).toHaveBeenCalledWith(newError.line, 1, newError.line, 1)
    })
  ...
  test('sets user awareness with quirky guest name when user is not signed in', async () => {
    ...
    render(
      <UserContext.Provider
        value={{
          context: { sessionActive: true, userId: 'guest-123' },
        }}
      >
        <UmlEditor
          roomId='testRoom'
  ...
  test('generates different colors for different usernames', async () => {
    ...
    const { unmount } = render(
      <UserContext.Provider
        value={{
          context: { displayName: 'User1', sessionActive: true },
        }}
      >
        <UmlEditor
          roomId='testRoom'
    ...
    render(
      <UserContext.Provider
        value={{
          context: { displayName: 'User2', sessionActive: true },
        }}
      >
        <UmlEditor
          roomId='testRoom'
