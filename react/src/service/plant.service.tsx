...
export const createDocumentInRoom = (
  ...
  socket.emit('/create', { documentName: documentName }, (response: any) => {
...
export const deleteDocumentInRoom = (
  ...
  socket.emit('/delete', { documentId: documentId }, (response: any) => {
    if (response.status === 'SUCCESS') {
...
const refreshToken = async (token: string | null) => {
  if (!token)
    throw new Error('Failed to refresh user session. Provided token is empty')
...
export const retrieveToken = async (
  loginGuestUserCallback?: (...args: any[]) => Promise<void> | undefined,
  ...args: any[]
) => {
  let token = window.localStorage.getItem('jwt') as string

  if (!token) {
    if (!loginGuestUserCallback)
      throw new Error('User session was not properly initialized')
...
export const getPublicRoom = async (roomName: string) => {
  ...
  const response = await fetch(`${serverHttpUrl}/room/public/${roomName}`, {
    headers: {
      Authorization: token,
    },
  })
...
export const shareRoom = async (roomId: string) => {
  const token = await retrieveToken()
  const response = await fetch(`${serverHttpUrl}/room/share/${roomId}`, {
    method: 'PUT',
    headers: {
      Authorization: token,
    },
...
export const changeRoomAccess = async (roomId: string, isPrivate: boolean) => {
  const token = await retrieveToken()
  const response = await fetch(`${serverHttpUrl}/room/${roomId}/access`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
      Authorization: token,
