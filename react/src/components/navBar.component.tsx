...
export const NavBar: React.FC<NavBarProps> = ({ isPrivate, roomId, roomName }) => {
  ...
  const handleAccessToggle = async () => {
    ...
    try {
      const navigateLink = !isPrivate ? `/private/${userContext.context.userId}/${roomName}` : `/${roomName}`;
      setIsLoading(true);
    ...
    <div className="bg-slate-900 w-full h-16 flex items-center px-8 justify-between">
      ...
      <div className="flex items-center gap-2">
        ...
        <Button 
          ...
          disabled={isLoading}
          className={`${
            isPrivate ? 'bg-blue-500' : 'bg-green-500'
          }`}
        >
          <FontAwesomeIcon className="mr-2" icon={isPrivate ? faLock : faLockOpen} />
            {isLoading ? 'Changing...' : (isPrivate ? 'Private Room' : 'Public Room')}
          </Button>
        )}
      </div>
    </div>
