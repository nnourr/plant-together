...
export const TeamModal: React.FC<TeamModalProps> = ({ onClose, className }) => {
  const profiles = contributors.map((contributor, index) => (
    <a
      className={`flex gap-2 items-center w-32 flex-col text-white text-center in-hover:scale-[200%]`}
      key={index}
      href={contributor.link}
      target="__blank"
    >
      <div
        className={`rounded-full w-full aspect-square overflow-hidden border-transparent border-2 in-hover:border-blue-500 transition-all`}
      >
        <img
  ...
  return (
    ...
    >
      <div className="bg-slate-800 p-6 rounded-xl shadow-xl w-fit">
        ...
        <div className="flex gap-8 justify-between group/profile">
          {profiles}
        </div>
      </div>
    </div>
